...
# Validateur commun pour les numéros sénégalais
...
class CleanUserFieldsMixin:
    ...
    def clean_phone(self):
        ...
        if val:
            ...
            if len(val) != 9:
                raise forms.ValidationError("Le numéro doit contenir exactement 9 chiffres.")
...
class ApporteurCreationForm(UserCreationForm, CleanUserFieldsMixin):
    ...
    class Meta:
        ...
        fields = ['username', 'first_name', 'last_name', 'email', 'phone',
                  'address', 'grade', 'password1', 'password2']
        widgets = {
...
class ProfileUpdateForm(forms.ModelForm, CleanUserFieldsMixin):
    ...
    class Meta:
        ...
        widgets = {
            'first_name': forms.TextInput(attrs={'class': 'w-full px-4 py-2 border border-gray-600 rounded-lg bg-gray-800 text-gray-100'}),
            'last_name': forms.TextInput(attrs={'class': 'w-full px-4 py-2 border border-gray-600 rounded-lg bg-gray-800 text-gray-100'}),
            'email': forms.EmailInput(attrs={'class': 'w-full px-4 py-2 border border-gray-600 rounded-lg bg-gray-800 text-gray-100'}),
            'address': forms.Textarea(attrs={'class': 'w-full px-4 py-2 border border-gray-600 rounded-lg bg-gray-800 text-gray-100', 'rows': 3}),
        }
...
class AdminApporteurUpdateForm(forms.ModelForm, CleanUserFieldsMixin):
    ...
    class Meta:
        model = User
        fields = ['first_name', 'last_name', 'email', 'phone', 'address', 'grade', 'is_active']
        widgets = {
            'first_name': forms.TextInput(attrs={'class': 'w-full px-4 py-2 border border-gray-600 rounded-lg bg-gray-800 text-gray-100'}),
            'last_name': forms.TextInput(attrs={'class': 'w-full px-4 py-2 border border-gray-600 rounded-lg bg-gray-800 text-gray-100'}),
            'email': forms.EmailInput(attrs={'class': 'w-full px-4 py-2 border border-gray-600 rounded-lg bg-gray-800 text-gray-100'}),
            'address': forms.Textarea(attrs={'class': 'w-full px-4 py-2 border border-gray-600 rounded-lg bg-gray-800 text-gray-100', 'rows': 3}),
            'grade': forms.Select(attrs={'class': 'w-full px-4 py-2 border border-gray-600 rounded-lg bg-gray-800 text-gray-100'}),
            'is_active': forms.CheckboxInput(attrs={'class': 'w-4 h-4 text-blue-600 bg-gray-800 border-gray-600 rounded focus:ring-blue-500'})
        }
...
class CustomLoginForm(forms.Form):
    ...
    remember_me = forms.BooleanField(
        ...
        required=False,
        widget=forms.CheckboxInput(attrs={'class': 'w-4 h-4 text-blue-600 bg-gray-800 border-gray-600 rounded'})
    )
    ...
    def clean(self):
        ...
        if username and password:
            # Essayer username
            self.user_cache = authenticate(self.request, username=username.lower().strip(), password=password)
            # Sinon email
...
class BulkActionForm(forms.Form):
    ...
    action = forms.ChoiceField(
        ...
        required=True,
        widget=forms.Select(attrs={'class': 'w-full px-4 py-2 border border-gray-600 rounded-lg bg-gray-800 text-gray-100'})
    )
...
class SearchApporteurForm(forms.Form):
    ...
    status = forms.ChoiceField(
        required=False,
        choices=[('', 'Tous les statuts'), ('actif', 'Actifs'), ('inactif', 'Inactifs')],
        widget=forms.Select(attrs={'class': 'w-full px-4 py-2 border border-gray-600 rounded-lg bg-gray-800 text-gray-100'})
    )
    date_creation = forms.DateField(
        required=False,
        widget=forms.DateInput(attrs={'type': 'date', 'class': 'w-full px-4 py-2 border border-gray-600 rounded-lg bg-gray-800 text-gray-100'})
    )
