...
def _validate_immatriculation(immat: str) -> str: # CORRECTION 1: Suppression de 'self'
    """
...
class AskiaAPIClient:
    ...
    def create_contrat_auto(
        ...
        try:
            immat = _validate_immatriculation(contrat_data["immatriculation"]) # CORRECTION 2: Suppression de 'self.'
...
class AskiaAPIClient:
    ...
    # ========================================================================
    # NOUVELLE MÉTHODE DE RENOUVELLEMENT
    # ========================================================================
    def renew_contrat_auto(
        ...
        """
        Renouvelle un contrat automobile via l'endpoint srwbauto/renouv.
        """
        params = {
            "cliCode": cli_code,
        ...
        logger.info(
            "Tentative renouvellement contrat | Client=%s | Police=%s | Effet=%s",
            cli_code,
            numero_police,
            effet
        )
        ...
        try:
            # Utiliser un timeout plus long pour les opérations de renouvellement/création
            result = self._make_request(
                "srwbauto/renouv",
                method="GET",
                params=params,
                timeout=90,
                allow_retry=False,  # Ne pas réessayer automatiquement une transaction financière
            )

            logger.info(
                "✅ Renouvellement réussi | Police=%s | Facture=%s",
                result.get("numeroPolice"),
                result.get("numeroFacture")
            )
            return result

        except Exception as e:
            logger.error(
                "Échec renouvellement contrat | Police=%s | Erreur=%s",
                numero_police,
                e
            )
            # Renvoyer l'exception pour que la vue puisse l'attraper
            raise
    ...
    def get_referentiel_sous_categories(
        ...
        try:
            result = self._make_request("referentiel/scategories", params=params)
            # Gérer le cas où l'API retourne une liste vide ou None
            if not result:
                return []
            return [(sc["code"], sc["libelle"]) for sc in result]
        except Exception:
