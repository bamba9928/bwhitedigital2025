...
class Client(models.Model):
    ...
    adresse = models.CharField(
        ...
        validators=[
            MinLengthValidator(2, message="Adresse trop courte (2 caractères minimum)")
        ],
    )
...
class Vehicule(models.Model):
    ...
    categorie = models.CharField(
        max_length=3, choices=CATEGORIES, verbose_name="Catégorie"
    )
    ...
    carburant = models.CharField(
        max_length=6, choices=CARBURANTS, verbose_name="Type de carburant"
    )
...
class ContratQuerySet(models.QuerySet):
    def emis_avec_doc(self):
        ...
        return self.filter(status__in=["EMIS", "ACTIF", "EXPIRE"]).filter(
...
class ContratManager(Manager):
    ...
    def emis_avec_doc(self):
        ...
        self.get_queryset().filter(
            status__in=["EMIS", "ACTIF"], date_echeance__lt=date.today()
...
class Contrat(models.Model):
    ...
    numero_facture = models.CharField(
        max_length=50, blank=True, null=True, verbose_name="N° Facture Askia"
    )
    date_effet = models.DateField()
    duree = models.PositiveIntegerField(choices=DUREE_CHOICES, default=12)
    date_echeance = models.DateField(blank=True, null=True)
    type_garantie = models.CharField(
        max_length=100, default="Responsabilité Civile", verbose_name="Type de Garantie"
    )
    ...
    commission_apporteur = models.DecimalField(
        max_digits=10, decimal_places=2, default=0
    )
    commission_bwhite = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        default=0,
        verbose_name="Commission BWHITE (Profit)",
    )
    net_a_reverser = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        default=0,
    ...
    link_facture = models.URLField(null=True, blank=True)

    # Champs d'annulation
    annule_at = models.DateTimeField(
    ...
    annule_raison = models.CharField(
        max_length=255, blank=True, verbose_name="Raison d'annulation"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                check=Q(prime_nette__gte=0), name="prime_nette_non_negative"
            ),
            models.CheckConstraint(
                check=Q(prime_ttc__gte=0), name="prime_ttc_non_negative"
            ),
        ]
